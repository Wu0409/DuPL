...
parser.add_argument("--comment", default='_train_voc', type=str, help="comment")
...
# DIR
parser.add_argument("--work_dir", default="work_dir_voc", type=str, help="work_dir_voc_wseg")
...
# TRAINING
parser.add_argument("--samples_per_gpu", default=2, type=int, help="samples_per_gpu")
...
parser.add_argument("--w_seg_diff", default=2.0, type=float, help="w_seg")

# GMM
parser.add_argument("--gmm_iters", default=8000, type=int)
...
def train(args=None):
    ...
    train_dataset = voc.VOC12ClsDataset(
        root_dir=args.data_folder, name_list_dir=args.list_folder, split=args.train_set, stage='train', aug=True,
        rescale_range=args.scales, crop_size=args.crop_size, img_fliplr=True, ignore_index=args.ignore_index,
        num_classes=args.num_classes,
    )
    train_sampler = DistributedSampler(train_dataset, shuffle=True)
    train_loader = DataLoader(
        train_dataset, batch_size=args.samples_per_gpu, shuffle=False, num_workers=args.num_workers,
        pin_memory=False, drop_last=True, sampler=train_sampler, prefetch_factor=4
    )
    train_sampler.set_epoch(np.random.randint(args.max_iters))
    train_loader_iter = iter(train_loader)

    val_dataset = voc.VOC12SegDataset(
        root_dir=args.data_folder, name_list_dir=args.list_folder, split=args.val_set, stage='val', aug=False,
    ...
    par = PAR(num_iter=10, dilations=[1, 2, 4, 8, 12, 24]).cuda()  # post-processing

    high_thres_start = torch.ones(20, device=device) * args.high_thre
    high_thres_target = torch.tensor([
        0.70, 0.70, 0.70, 0.70, 0.55, 0.55, 0.55, 0.55, 0.70, 0.55,
        0.55, 0.55, 0.55, 0.55, 0.55, 0.55, 0.55, 0.55, 0.70, 0.55,
    ]).to(device)
    ...
    for n_iter in range(args.max_iters):
        ...
        if n_iter < args.cam_iters:
            ...
            _, pseudo_label_aux_1 = cam_helper.cam_to_label(
            ...
            _, pseudo_label_aux_2 = cam_helper.cam_to_label(
            ...
            high_thres = train_helper.cosine_descent(
                high_thres_start, high_thres_target, n_iter - args.cam_iters, args.max_iters - args.cam_iters
            )
        ...
        # ------------------------------------------------------------------------------------------------------
        if n_iter <= args.cam_iters:
            loss = 1.0 * cls_loss + args.w_ptc * ptc_loss + 0.0 * seg_loss + 0.1 * sim_loss
        elif n_iter <= args.gmm_iters:
            loss = 1.0 * cls_loss + args.w_ptc * ptc_loss + args.w_seg * seg_loss + 0.1 * sim_loss + 0.00 * reg_loss
        else:
            loss = 1.0 * cls_loss + args.w_ptc * ptc_loss + args.w_seg * seg_loss + 0.1 * sim_loss + 0.05 * reg_loss
        ...
        if (n_iter + 1) % args.eval_iters == 0:
            ...
            if local_rank == 0:
                ...
                val_cls_score_1, val_cls_score_2, tab_results, score_item = train_helper.validate_siamase(
